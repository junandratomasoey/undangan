<section class="bg-cream-soft px-6 py-24">
    <header class="reveal text-center">
        ...
        <h2 class="font-display text-4xl text-maroon">Rangkaian Acara</h2>
    </header>
    <div class="mx-auto mt-14 grid max-w-3xl gap-8 sm:grid-cols-{{ min($inv->events->count(), 2) }}">
        @foreach($inv->events as $event)
            <article class="reveal border-2 border-ochre/30 bg-cream p-8 text-center">
                <h3 class="font-display text-3xl text-maroon">{{ $event->title }}</h3>
                <div class="mx-auto my-5 w-24 text-ochre"><div class="tenun-band tenun-band--thin"></div></div>
                ...
                <p class="mt-5 font-display text-xl text-maroon">{{ $event->venue_name }}</p>
                @if($event->address)
                ...
                @if($inv->hasFeature('maps_link') && ($event->maps_url || $event->hasCoordinates()))
                    <a href="{{ $event->maps_url ?: 'https://www.google.com/maps/search/?api=1&query='.$event->lat.','.$event->lng }}"
                       ...
                       class="mt-6 inline-flex items-center gap-2 rounded-sm border border-maroon/40 px-5 py-2 font-sans text-xs uppercase tracking-[0.15em] text-maroon transition hover:bg-maroon hover:text-cream">
