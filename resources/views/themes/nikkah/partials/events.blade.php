<section class="bg-ivory-warm px-6 py-24">
    <header class="reveal text-center">
        ...
        <h2 class="mt-5 font-display text-5xl text-emerald2-deep">Rangkaian Acara</h2>
    </header>
    <div class="mx-auto mt-14 grid max-w-3xl gap-8 sm:grid-cols-{{ min($inv->events->count(), 2) }}">
        @foreach($inv->events as $event)
            <article class="reveal mihrab overflow-hidden border border-emerald2/25 bg-ivory px-8 pb-9 pt-14 text-center shadow-sm">
                <h3 class="font-display text-3xl text-emerald2-deep">{{ $event->title }}</h3>
                <div class="rule-gold mx-auto my-5 flex items-center justify-center gap-3"></div>
                ...
                <p class="mt-5 font-display text-2xl text-emerald2-deep">{{ $event->venue_name }}</p>
                @if($event->address)
                ...
                @if($inv->hasFeature('maps_link') && ($event->maps_url || $event->hasCoordinates()))
                    <a href="{{ $event->maps_url ?: 'https://www.google.com/maps/search/?api=1&query='.$event->lat.','.$event->lng }}"
                       ...
                       class="mt-6 inline-flex items-center gap-2 rounded-full border border-emerald2/40 px-5 py-2 font-sans text-[10px] uppercase tracking-[0.15em] text-emerald2-deep transition hover:bg-emerald2-deep hover:text-ivory">
