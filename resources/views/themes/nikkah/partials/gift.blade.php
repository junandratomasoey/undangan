<section class="px-6 py-24" x-data="{ copied:null }">
    <div class="mx-auto max-w-xl">
        <header class="reveal text-center">
            ...
            <h2 class="mt-4 font-display text-5xl text-emerald2-deep">Tanda Kasih</h2>
        ...
        <div class="mt-10 space-y-4">
            @foreach($inv->giftAccounts as $gift)
                <div class="reveal border border-emerald2/25 bg-ivory-warm p-6">
                    @if($gift->kind === 'qris')
                        <p class="text-center font-display text-2xl text-emerald2-deep">{{ $gift->label }}</p>
                        @if($gift->qris_image)
                    ...
                    @elseif($gift->kind === 'address')
                        <p class="font-display text-xl text-emerald2-deep">{{ $gift->label }}</p>
                    ...
                    @else
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-1 font-display text-2xl tracking-wide text-emerald2-deep">{{ $gift->account_number }}</p>
                            ...
                            <button @click="navigator.clipboard.writeText('{{ $gift->account_number }}'); copied='{{ $gift->id }}'; setTimeout(()=>copied=null,1800)"
                                    class="shrink-0 rounded-full border border-emerald2/40 px-4 py-2 font-sans text-[10px] uppercase tracking-[0.12em] text-emerald2-deep transition hover:bg-emerald2-deep hover:text-ivory">
