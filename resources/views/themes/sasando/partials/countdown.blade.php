<section class="bg-forest-deep px-6 py-24 text-ivory"
         x-data="countdown('{{ $event->starts_at->toIso8601String() }}')" x-init="start()">
    <div class="reveal mx-auto max-w-2xl text-center">
        <p class="font-sans text-xs uppercase tracking-[0.25em] text-sage">Menuju Hari Bahagia</p>
        <div class="mt-8 grid grid-cols-4 gap-3 sm:gap-6">
            @foreach(['days'=>'Hari','hours'=>'Jam','minutes'=>'Menit','seconds'=>'Detik'] as $key => $label)
                <div class="rounded-sm border border-accent/30 bg-ivory/5 py-5">
                    <div class="font-display text-4xl text-accent sm:text-5xl" x-text="String(t.{{ $key }}).padStart(2,'0')">00</div>
